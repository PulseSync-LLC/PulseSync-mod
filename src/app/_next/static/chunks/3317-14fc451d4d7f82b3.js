(self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([
    [3317],
    {
        6820: (t) => {
            t.exports = {
                root: 'ArtistAboutModal_root__bmUo9',
                modalContent: 'ArtistAboutModal_modalContent__RGkJk',
                header: 'ArtistAboutModal_header__yLnAj',
                overlay: 'ArtistAboutModal_overlay__6hToT',
                closeButton: 'ArtistAboutModal_closeButton__Gnz25',
            };
        },
        7474: (t, e, r) => {
            'use strict';
            r.d(e, { ArtistCard: () => C });
            var o = r(32290),
                i = r(63618),
                a = r(96103),
                l = r(55178),
                n = r(60900),
                s = r(16172),
                c = r(59824),
                u = r(71483),
                d = r(91027),
                _ = r(6752),
                m = r(14934),
                p = r(82586),
                v = r(86269),
                f = r(71926),
                A = r(33797),
                g = r(49574),
                x = r(44574),
                h = r(21564),
                b = r(42048),
                T = r.n(b);
            let C = (0, a.PA)((t) => {
                let { artist: e, className: r, children: a, contentLinesCount: b, topTitleElement: C, bottomTitleElement: y } = t,
                    { ref: j, intersectionPropertyId: k } = (0, g.nMI)(),
                    {
                        trailer: N,
                        user: M,
                        paywall: { modal: w },
                    } = (0, g.Pjs)(),
                    { from: I, utmLink: S } = (0, g.fyy)({ contextId: e.id, contextType: u.K.Artist }),
                    { formatMessage: E } = (0, n.A)(),
                    [L, R] = (0, l.useState)(!1),
                    [O, B] = (0, l.useState)(!1),
                    [P, K] = (0, l.useState)(!1),
                    { sendLikeSearchFeedback: D, sendNavigateSearchFeedback: z, sendPlaySearchFeedback: F } = (0, g.zEv)(),
                    U = (0, g.NKK)(),
                    H = (0, g.brA)(),
                    W = (0, A.KX)(e),
                    q = (0, A.A7)(e),
                    { id: X, name: G, coverUri: Y, isLiked: J } = e,
                    Z = (0, g.ZpR)(e.url),
                    [$, Q] = (0, l.useState)(!1),
                    V = (0, g.Ftl)(),
                    tt = (0, g.PT7)(),
                    te = (0, d.c)((t) => {
                        if ((t.stopPropagation(), tt())) return void t.preventDefault();
                        N.openArtistTrailer(e.id), V(s.ky.Artist, e.id);
                    }),
                    tr = (0, l.useMemo)(() => {
                        let t = E({ id: 'entity-names.artist-name' }, { artistName: G }),
                            e = J ? E({ id: 'entity-names.has-your-like' }) : '';
                        return ''.concat(t, ' ').concat(e);
                    }, [G, J, E]),
                    { isPlaying: to, togglePlay: ti } = (0, g.Dx4)({
                        playContextParams: { contextData: { type: u.K.Artist, meta: { id: Number(X) }, from: I, utmLink: S }, loadContextMeta: !0 },
                    }),
                    ta = (0, A.SA)({ artist: e, callback: Z }),
                    tl = (0, A.SA)({ artist: e, callback: ti }),
                    tn = (0, d.c)((t) => {
                        null == z || z(), U({ to: s.QT.ArtistScreen }), ta(t);
                    }),
                    ts = (0, g.NPu)(),
                    tc = (0, d.c)(() => {
                        if (!tt()) {
                            if (ts) return void w.open();
                            L || to || (R(!0), null == F || F()), tl(), H(!to);
                        }
                    }),
                    tu = (0, d.c)(() => {
                        O || J || (B(!0), null == D || D()), W();
                    }),
                    td = (0, d.c)((t) => {
                        t.preventDefault(), t.stopPropagation();
                    }),
                    t_ = (0, d.c)((t) => {
                        K(t), Q(t);
                    }),
                    tm = (0, l.useMemo)(
                        () =>
                            (0, o.jsx)(
                                h.ArtistContextMenu,
                                {
                                    artist: e,
                                    onOpenChange: t_,
                                    open: P,
                                    onClick: td,
                                    className: (0, i.$)(T().menuButton, T().control),
                                    size: 's',
                                    icon: (0, o.jsx)(p.Icon, { size: 'xxs', variant: 'more' }),
                                    ...(0, c.Am)(c.Kq.artist.ARTIST_CONTEXT_MENU_BUTTON),
                                },
                                e.getKey('ArtistContextMenu'),
                            ),
                        [e, td, t_, P],
                    ),
                    tp = (0, l.useMemo)(() => {
                        var t;
                        if (null == e || null == (t = e.trailer) ? void 0 : t.isAvailable)
                            return (0, o.jsx)(
                                x.nE,
                                {
                                    children: (0, o.jsx)(x.kJ, {
                                        className: (0, i.$)(T().trailerButton, T().control),
                                        radius: 'round',
                                        size: 's',
                                        iconSize: 'xxs',
                                        onClick: te,
                                    }),
                                },
                                e.getKey('ArtistCardTrailerTooltip'),
                            );
                    }, [e, te]),
                    tv = (0, l.useMemo)(
                        () =>
                            (0, o.jsx)(
                                x.O0,
                                { onClick: q, isPinned: e.isPinned, className: (0, i.$)(T().pinButton, T().control), withRipple: !1 },
                                e.getKey('PinButton'),
                            ),
                        [e, q],
                    ),
                    tf = (0, _.L)(() => {
                        if (e.isAvailable)
                            return (0, o.jsx)(
                                m.hg,
                                {
                                    isVisible: P || $,
                                    className: T().controls,
                                    radius: 'round',
                                    playControl: (0, o.jsx)(
                                        x.DM,
                                        {
                                            buttonVariant: 'default',
                                            withHover: !1,
                                            className: (0, i.$)(T().playButton, T().control),
                                            iconSize: 'xl',
                                            variant: 'filled',
                                            onClick: tc,
                                            isPlaying: to,
                                            disabled: !e.isAvailableForPlaying,
                                        },
                                        e.getKey('PlayButton'),
                                    ),
                                    likeControl: (0, o.jsx)(
                                        x.cy,
                                        {
                                            className: (0, i.$)(T().likeButton, T().control),
                                            isLiked: J,
                                            onClick: tu,
                                            variant: 'default',
                                            size: 's',
                                            iconSize: 'xxs',
                                            disabled: !M.isAuthorized,
                                        },
                                        e.getKey('LikeButton'),
                                    ),
                                    menuControl: tm,
                                    pinControl: tv,
                                    trailerControl: tp,
                                },
                                e.getKey('ArtistCardControls'),
                            );
                    }),
                    tA = (0, l.useMemo)(
                        () =>
                            (0, o.jsx)(v.Paper, {
                                className: T().cover,
                                radius: 'round',
                                withShadow: !0,
                                ...(0, c.Am)(c.Kq.artist.ARTIST_CARD),
                                children: (0, o.jsxs)('div', {
                                    className: T().coverBlock,
                                    onClick: tn,
                                    children: [
                                        (0, o.jsx)(x.BW, {
                                            className: T().image,
                                            src: Y,
                                            size: 200,
                                            fit: 'cover',
                                            alt: tr,
                                            withAvatarReplace: !0,
                                            isAvailable: e.isAvailable,
                                            'aria-hidden': !0,
                                        }),
                                        tf,
                                    ],
                                }),
                            }),
                        [tn, Y, tr, e.isAvailable, tf],
                    );
                return (0, o.jsx)(m.MN, {
                    ref: j,
                    className: (0, i.$)(T().root, r),
                    textPosition: 'center',
                    'aria-label': tr,
                    title: (0, o.jsxs)(o.Fragment, {
                        children: [
                            C,
                            (0, o.jsx)(f.Caption, {
                                variant: 'div',
                                type: 'entity',
                                size: 's',
                                weight: 'medium',
                                lineClamp: 2,
                                children: (0, o.jsx)(x.N_, {
                                    className: T().titleLink,
                                    href: e.url,
                                    'aria-label': tr,
                                    onClick: tn,
                                    ...(0, c.Am)(c.Kq.artist.ARTIST_TITLE),
                                    children: G,
                                }),
                            }),
                            y,
                        ],
                    }),
                    srTitle: (0, o.jsx)(x.N_, { href: e.url, onClick: tn, children: tr }),
                    'data-intersection-property-id': k,
                    contentLinesCount: b,
                    view: tA,
                    ...(0, c.Am)(c.Kq.artist.ARTIST_ITEM),
                    children: a,
                });
            });
        },
        8092: (t, e, r) => {
            'use strict';
            r.d(e, { K: () => u });
            var o,
                i = r(97020),
                a = r(55178),
                l = {
                    5881: (t, e, r) => {
                        function o() {
                            for (var t, e, r = 0, o = ''; r < arguments.length; )
                                (t = arguments[r++]) &&
                                    (e = (function t(e) {
                                        var r,
                                            o,
                                            i = '';
                                        if ('string' == typeof e || 'number' == typeof e) i += e;
                                        else if ('object' == typeof e)
                                            if (Array.isArray(e)) for (r = 0; r < e.length; r++) e[r] && (o = t(e[r])) && (i && (i += ' '), (i += o));
                                            else for (r in e) e[r] && (i && (i += ' '), (i += r));
                                        return i;
                                    })(t)) &&
                                    (o && (o += ' '), (o += e));
                            return o;
                        }
                        r.r(e), r.d(e, { clsx: () => o, default: () => i });
                        let i = o;
                    },
                    7122: (t, e, r) => {
                        r.r(e), r.d(e, { default: () => o });
                        let o = {
                            contentContainer: 'JjlbHZ4FaP9EAcR_1DxF',
                            contentContainer_block: 'iOlzvyUREgDkthkrx7Sf',
                            flexIcon: 'WsKeF73pWotx9W1tWdYY',
                            root: 'cpeagBA1_PblpJn8Xgtv',
                            root_withoutBorder: 'qU2apWBO1yyEK0lZ3lPO',
                            root_radius_xs: 'MmZbSs387gu2qrJ1lDWd',
                            root_radius_s: 'mlcrraysn7mW6xrBXSBF',
                            root_radius_m: 'dgV08FKVLZKFsucuiryn',
                            root_radius_l: 'S97_5dtzhpxo_amtfYRg',
                            root_radius_xl: 'nNBJnDHRlyLTniWosJhk',
                            root_radius_xxxl: 'zIMibMuH7wcqUoW7KH1B',
                            root_radius_round: 'uwk3hfWzB2VT7kE13SQk',
                            root_size: 'IlG7b1K0AD7E7AMx6F5p',
                            root_size_default: 'C_QGmfTz6UFX93vfPt6Z',
                            root_size_xxxs: 'eQt33MLDiQ6DRSuLaYEp',
                            root_size_xxs: 'oR11LfCBVqMbUJiAgknd',
                            root_size_xs: 'j1jXIVckFgZECecFzZMe',
                            root_size_s: 'WtFdWDF44egSVM_YiMUX',
                            root_size_m: 'Y2uqxoU7xa_AZ8FUCVOW',
                            root_size_l: 'SGYcNjvjmMsXeEVGUV2Z',
                            root_icon_left: 'kc5CjvU5hT9KEj0iTt3C',
                            root_icon_right: 'et24Jf7pT_X9Fvc7TznR',
                            root_primary: 'fXlFz1qMkliFUWOkHo8T',
                            root_primary_default: '_eTRQi5ADZCUvUKMZqJU',
                            ripple: 'spMT3NcRD9Yb0ntNaNct',
                            root_primary_outline: 'fCUSh2B0Ye9kEvceE8zc',
                            root_primary_text: 'qlPp6CSQQEMVZPqtqLiQ',
                            root_primary_withHover: 'KZF6_4K1p_Y_GMIAxaAn',
                            root_primary_withHover_default: 'rWukOKAJh5Ga7JuIp62L',
                            root_primary_withHover_outline: 'fdwWCJKgUqml5wNqrRcN',
                            root_primary_withHover_text: 'IgYbZLnYjW0nMahgpkus',
                            root_secondary: '_T4p_w41oaq6L4sztSdw',
                            root_secondary_default: 'iJVAJMgccD4vj4E4o068',
                            root_secondary_outline: 'pnM3iSP9keZOELI2oohr',
                            root_secondary_text: 'UDMYhpDjiAFT3xUx268O',
                            root_secondary_withHover: 'qUbrkhZIOVrvM0roV1QF',
                            root_secondary_withHover_default: 'nHWc2sto1C6Gm0Dpw_l0',
                            root_secondary_withHover_outline: 'i5WuBm5mfG0mflk_1jH_',
                            root_secondary_withHover_text: 'HbaqudSqu7Q3mv3zMPGr',
                            root_plus: 'ixLRsIJ2FvXO2k04n_QY',
                            root_plus_default: 'yRHwHzEGfDgRXGzYJqw2',
                            root_plus_outline: 'e777irPFmyQFFrURLF_U',
                            root_plus_text: 'vRqDhvmt3gt8TFp45_Zw',
                            root_plus_withHover: 'TZif6q3I2RwBEYXwK_iA',
                            root_plus_withHover_default: 'k3DhvmzpnM_Fb9oFdE4q',
                            root_plus_withHover_outline: 'RiDWYwGIxqbrUPR699DM',
                            root_plus_withHover_text: 'hC_mMCzWjkTn2j9xZzGc',
                            root_accent: 'jqD2jMT6n7F0WKyqwMsn',
                            root_accent_default: 'bDp0r9MtoYECZ8ObMoCh',
                            root_accent_outline: 'rXNyGp8NBAw2MUjACZNj',
                            root_accent_text: 'hmV4ERaXWAJc4uPLZL30',
                            root_accent_withHover: 'uKuxXu1N4TP5cWaEK5Ke',
                            root_accent_withHover_default: 't_hequUaUgAMhFuxizLb',
                            root_accent_withHover_outline: 'Oy9sPFTxNTo1_E29U4aF',
                            root_accent_withHover_text: 'LcKRSd3DLoh7k60Oqox8',
                            root_withActiveSpinner: 'nAGvO87rLs15SJgft6Hh',
                            block: 'BbCxxIjBGupN28bq2lSP',
                            icon: 'J9wTKytjOWG73QMoN5WP',
                            icon_position_left: 'elJfazUBui03YWZgHCbW',
                            icon_position_right: 'RBoEbyJKP5rEtLsXM1ji',
                            icon_withButtonSize: 'cE17_kCWJgx8kzQEkeVr',
                            spinnerContainer: 'STbBDGqYjUEcLuNvhu9w',
                        };
                    },
                    4894: (t, e, r) => {
                        r.r(e), r.d(e, { default: () => o });
                        let o = {
                            root: 'BnN6sQIg6NahNBun6fkP',
                            fade: 'MM8MKXCw0gMkVvq7C1YS',
                            fade_active: 'MsLY_qiKofQrwKAr98EC',
                            button: 'Dp6n_Y0cfUyPQT1Z6uIm',
                            text: 'bfmUuyonXAK7HKYtDzUK',
                        };
                    },
                    9097: (t, e) => {
                        var r = Symbol.for('react.transitional.element');
                        function o(t, e, o) {
                            var i = null;
                            if ((void 0 !== o && (i = '' + o), void 0 !== e.key && (i = '' + e.key), 'key' in e))
                                for (var a in ((o = {}), e)) 'key' !== a && (o[a] = e[a]);
                            else o = e;
                            return { $$typeof: r, type: t, key: i, ref: void 0 !== (e = o.ref) ? e : null, props: o };
                        }
                        (e.Fragment = Symbol.for('react.fragment')), (e.jsx = o), (e.jsxs = o);
                    },
                    4377: (t, e, r) => {
                        t.exports = r(9097);
                    },
                    4601: (t, e, r) => {
                        Object.defineProperty(e, '__esModule', { value: !0 }), (e.useToggle = void 0);
                        let o = r(810);
                        e.useToggle = (t) => {
                            let [e, r] = (0, o.useState)(t);
                            (0, o.useEffect)(() => {
                                r(t);
                            }, [t]);
                            let i = (0, o.useCallback)(() => {
                                    r((t) => !t);
                                }, []),
                                a = (0, o.useCallback)(() => {
                                    r(!0);
                                }, []),
                                l = (0, o.useCallback)(() => {
                                    r(!1);
                                }, []);
                            return { state: e, toggle: i, setState: r, toggleTrue: a, toggleFalse: l };
                        };
                    },
                    666: (t, e, r) => {
                        Object.defineProperty(e, '__esModule', { value: !0 }), (e.useTruncate = void 0);
                        let o = r(352),
                            i = r(810),
                            a = r(4601);
                        e.useTruncate = (t) => {
                            let [e, r] = (0, i.useState)(0),
                                { state: l, toggleTrue: n, toggleFalse: s } = (0, a.useToggle)(!1),
                                c = (0, i.useMemo)(
                                    () =>
                                        (0, o.throttle)(() => {
                                            let { offsetHeight: r, scrollHeight: o } = (null == t ? void 0 : t.current) || {};
                                            r && o && e < o ? n() : s();
                                        }, 100),
                                    [e, t, s, n],
                                );
                            return (
                                (0, i.useEffect)(() => {
                                    var o, i;
                                    !e && (null == (o = t.current) ? void 0 : o.offsetHeight) && r(null == (i = t.current) ? void 0 : i.offsetHeight);
                                }, [e, t]),
                                (0, i.useEffect)(
                                    () => (
                                        c(),
                                        window.addEventListener('resize', c),
                                        () => {
                                            window.removeEventListener('resize', c);
                                        }
                                    ),
                                    [c],
                                ),
                                { isTruncated: l }
                            );
                        };
                    },
                    8796: (t, e) => {
                        Object.defineProperty(e, '__esModule', { value: !0 }),
                            (e.createRipple = void 0),
                            (e.createRipple = function (t, e, r) {
                                let o = null != r ? r : t.currentTarget,
                                    i = document.createElement('span'),
                                    a = Math.max(o.clientWidth, o.clientHeight),
                                    l = a / 2,
                                    n = o.getBoundingClientRect(),
                                    s = 0 === t.clientX ? Math.round(n.width / 2) : t.clientX - n.left,
                                    c = 0 === t.clientY ? Math.round(n.height / 2) : t.clientY - n.top;
                                (i.style.width = ''.concat(a, 'px')),
                                    (i.style.height = ''.concat(a, 'px')),
                                    (i.style.left = 0 === t.clientX ? '0px' : ''.concat(s - l, 'px')),
                                    (i.style.top = ''.concat(c - l, 'px')),
                                    i.classList.add(e);
                                let u = o.getElementsByClassName(e)[0];
                                u && u.remove(), o.insertBefore(i, o.firstChild);
                            });
                    },
                    361: (t, e) => {
                        Object.defineProperty(e, '__esModule', { value: !0 }),
                            (e.getElementFromRefOrElement = void 0),
                            (e.getElementFromRefOrElement = (t) => {
                                if (void 0 !== t) {
                                    if (null === t || t instanceof HTMLElement) return t;
                                    if (null === t.current || t.current instanceof HTMLElement) return t.current;
                                }
                            });
                    },
                    8216: (t, e, r) => {
                        Object.defineProperty(e, '__esModule', { value: !0 }), (e.getElementFromRefOrElement = e.createRipple = void 0);
                        var o = r(8796);
                        Object.defineProperty(e, 'createRipple', {
                            enumerable: !0,
                            get: function () {
                                return o.createRipple;
                            },
                        });
                        var i = r(361);
                        Object.defineProperty(e, 'getElementFromRefOrElement', {
                            enumerable: !0,
                            get: function () {
                                return i.getElementFromRefOrElement;
                            },
                        });
                    },
                    8119: function (t, e, r) {
                        var o =
                            (this && this.__importDefault) ||
                            function (t) {
                                return t && t.__esModule ? t : { default: t };
                            };
                        Object.defineProperty(e, '__esModule', { value: !0 }), (e.Button = void 0);
                        let i = r(4377),
                            a = r(810),
                            l = r(5881),
                            n = r(8216),
                            s = o(r(7122)),
                            c = (t) => {
                                let {
                                        forwardRef: e,
                                        isBlock: r,
                                        iconPosition: o = 'left',
                                        children: c,
                                        className: u,
                                        color: d = 'secondary',
                                        flexIcon: _,
                                        icon: m,
                                        spinner: p,
                                        role: v,
                                        onClick: f,
                                        radius: A = 'm',
                                        size: g,
                                        type: x = 'button',
                                        variant: h = 'default',
                                        withRipple: b = !0,
                                        withHover: T = !0,
                                        withBorder: C = !1,
                                        disabled: y,
                                        iconClassName: j,
                                        contentContainerClassName: k,
                                        ...N
                                    } = t,
                                    M = (0, a.useId)(),
                                    w = !a.Children.toArray(c).filter(Boolean).length,
                                    I = 'left' === o,
                                    S = null,
                                    E = (0, a.isValidElement)(p);
                                if (m) {
                                    var L, R;
                                    S = (0, a.cloneElement)(m, {
                                        className: (0, l.clsx)(
                                            s.default.icon,
                                            {
                                                [s.default['icon_position_'.concat(o)]]: o && !w,
                                                [s.default.icon_withButtonSize]: !(null == (L = m.props) ? void 0 : L.size),
                                            },
                                            null == (R = m.props) ? void 0 : R.className,
                                            j,
                                        ),
                                        key: M,
                                    });
                                }
                                let O = (0, a.useMemo)(() => (E ? (0, i.jsx)('div', { className: s.default.spinnerContainer, children: p }) : null), [E, p]),
                                    B = (0, a.useCallback)(
                                        (t) => {
                                            E || (b && (0, n.createRipple)(t, s.default.ripple), null == f || f(t));
                                        },
                                        [E, f, b],
                                    );
                                return (0, i.jsx)('button', {
                                    ref: e,
                                    className: (0, l.clsx)(
                                        s.default.root,
                                        s.default['root_'.concat(d, '_').concat(h)],
                                        s.default['root_radius_'.concat(A)],
                                        s.default.root_size,
                                        {
                                            [s.default['root_'.concat(d, '_withHover_').concat(h)]]: T && !y && !E,
                                            [s.default['root_size_'.concat(g)]]: g,
                                            [s.default.root_withoutBorder]: !C,
                                            [s.default.root_withActiveSpinner]: E,
                                            [s.default.block]: r,
                                            [s.default.flexIcon]: _,
                                            [s.default.iconOnly]: w,
                                            [s.default.root_icon_left]: m && !w && I,
                                            [s.default.root_icon_right]: m && !w && !I,
                                        },
                                        u,
                                    ),
                                    type: x,
                                    role: v,
                                    onClick: B,
                                    ...N,
                                    disabled: y,
                                    'aria-live': E ? 'polite' : 'off',
                                    'aria-busy': E,
                                    children:
                                        m || E
                                            ? (0, i.jsxs)('span', {
                                                  className: (0, l.clsx)(s.default.contentContainer, { [s.default.contentContainer_block]: r }, k),
                                                  children: [m && I && S, !w && c, m && !I && S, O],
                                              })
                                            : c,
                                });
                            };
                        e.Button = (0, a.forwardRef)((t, e) => (0, i.jsx)(c, { forwardRef: e, ...t }));
                    },
                    217: function (t, e, r) {
                        var o =
                            (this && this.__importDefault) ||
                            function (t) {
                                return t && t.__esModule ? t : { default: t };
                            };
                        Object.defineProperty(e, '__esModule', { value: !0 }), (e.CollapsableText = void 0);
                        let i = r(4377),
                            a = r(5881),
                            l = r(810),
                            n = r(666),
                            s = r(8119),
                            c = o(r(4894));
                        e.CollapsableText = (t) => {
                            var e;
                            let {
                                    className: r,
                                    children: o,
                                    lineClamp: u,
                                    moreText: d,
                                    lessText: _,
                                    buttonClassName: m,
                                    buttonProps: p,
                                    withFade: v,
                                    initialOpen: f = !1,
                                    open: A,
                                    onOpenChange: g,
                                    ...x
                                } = t,
                                [h, b] = (0, l.useState)(f),
                                T = (0, l.useRef)(null),
                                { isTruncated: C } = (0, n.useTruncate)(T),
                                y = null != A ? A : h,
                                j = null != g ? g : b,
                                k = (0, l.cloneElement)(o, {
                                    ref: T,
                                    lineClamp: (!y && u) || void 0,
                                    className: (0, a.clsx)(null == (e = o.props) ? void 0 : e.className, c.default.text),
                                }),
                                N = (0, l.useMemo)(
                                    () => (v && C ? (0, i.jsx)('div', { className: (0, a.clsx)(c.default.fade, { [c.default.fade_active]: !y }), children: k }) : k),
                                    [C, y, k, v],
                                ),
                                M = (0, l.useCallback)(() => {
                                    j(!y);
                                }, [y, j]),
                                w = y ? _ : d;
                            return (0, i.jsxs)('div', {
                                className: (0, a.clsx)(c.default.root, r),
                                ...x,
                                children: [
                                    N,
                                    C &&
                                        w &&
                                        (0, i.jsx)(s.Button, {
                                            ...p,
                                            variant: 'text',
                                            withRipple: !1,
                                            onClick: M,
                                            className: (0, a.clsx)(c.default.button, m),
                                            color: 'primary',
                                            children: w,
                                        }),
                                ],
                            });
                        };
                    },
                    352: (t) => {
                        t.exports = i;
                    },
                    810: (t) => {
                        t.exports = o || (o = r.t(a, 2));
                    },
                },
                n = {};
            function s(t) {
                var e = n[t];
                if (void 0 !== e) return e.exports;
                var r = (n[t] = { exports: {} });
                return l[t].call(r.exports, r, r.exports, s), r.exports;
            }
            (s.d = (t, e) => {
                for (var r in e) s.o(e, r) && !s.o(t, r) && Object.defineProperty(t, r, { enumerable: !0, get: e[r] });
            }),
                (s.o = (t, e) => Object.prototype.hasOwnProperty.call(t, e)),
                (s.r = (t) => {
                    'undefined' != typeof Symbol && Symbol.toStringTag && Object.defineProperty(t, Symbol.toStringTag, { value: 'Module' }),
                        Object.defineProperty(t, '__esModule', { value: !0 });
                });
            var c = {};
            (() => {
                Object.defineProperty(c, 'X', { value: !0 }), (c.f = void 0);
                var t = s(217);
                Object.defineProperty(c, 'f', {
                    enumerable: !0,
                    get: function () {
                        return t.CollapsableText;
                    },
                });
            })();
            var u = c.f;
            c.X;
        },
        17596: (t) => {
            t.exports = {
                card_root: 'HorizontalCardContainer_root__YoAAP',
                root: 'ControlsBar_root__Cb_I5',
                item: 'ControlsBar_item__Bgdoc',
                item_buttonArrow: 'ControlsBar_item_buttonArrow__Hpg2f',
                controls: 'ControlsBar_controls__JCmC_',
                likeIcon: 'ControlsBar_likeIcon__LBoka',
                trailerIcon: 'ControlsBar_trailerIcon__W0fPg',
                controls_disabled: 'ControlsBar_controls_disabled__gLZiS',
            };
        },
        21564: (t, e, r) => {
            'use strict';
            r.d(e, { ArtistContextMenu: () => f });
            var o = r(32290),
                i = r(96103),
                a = r(60900),
                l = r(16172),
                n = r(59824),
                s = r(71483),
                c = r(91027),
                u = r(82586),
                d = r(19740),
                _ = r(33797),
                m = r(78822),
                p = r(49574),
                v = r(44574);
            let f = (0, i.PA)((t) => {
                var e, r, i;
                let { artist: f, onOpenChange: A, open: g, ...x } = t,
                    { shouldShowBuySubscriptionModal: h, showBuySubscriptionModal: b } = (0, p.qBP)(),
                    {
                        settings: { isMobile: T },
                        modals: { artistAboutModal: C },
                        trailer: y,
                        user: j,
                        experiments: k,
                    } = (0, p.Pjs)(),
                    N = (0, _.A7)(f),
                    M = (0, _.KX)(f),
                    w = (0, _.mW)(f),
                    I = (0, p.Ftl)(),
                    S = ''.concat(p.UfI.ARTIST, '-').concat(null == f ? void 0 : f.id),
                    { formatMessage: E } = (0, a.A)(),
                    { utmLink: L } = (0, p.fyy)({ blockId: p.UfI.ARTIST, contextType: s.K.Artist, contextId: null == f ? void 0 : f.id }),
                    { shareLink: R, pathname: O } = (0, p.bNS)('/artist/:artistId', { params: { artistId: null != (r = null == f ? void 0 : f.id) ? r : '' } }),
                    B = (0, m.A_)({ entityVariant: p.DwC.ARTIST, urlParams: { id: null == f ? void 0 : f.id } }),
                    { isPlaying: P, togglePlay: K } = (0, p.B0S)({
                        seeds: null != (i = null == f ? void 0 : f.seeds) ? i : [],
                        pageIdForFrom: p._Q$.RADIO,
                        blockIdForFrom: S,
                        parentContextId: null == f ? void 0 : f.id,
                    }),
                    D = (0, p.PT7)(),
                    z = E((null == f ? void 0 : f.isComposer) ? { id: 'artist.about-composer' } : { id: 'artist.about-artist' }),
                    F = (0, c.c)(() => {
                        if (h && j.isAuthorized) return void b();
                        P || K();
                    }),
                    U = (0, c.c)(() => {
                        if (!D()) {
                            if (h) return void b();
                            (null == f ? void 0 : f.id) && (y.setUtmLink(L), y.openArtistTrailer(f.id), I(l.ky.Artist, f.id));
                        }
                    }),
                    H = (0, c.c)(() => {
                        C.open(null == f ? void 0 : f.id);
                    });
                (0, p.NBO)(g);
                let W = { variant: p.Yxq.ARTIST, id: null == f ? void 0 : f.id, title: null == f ? void 0 : f.name, path: O },
                    q = k.checkExperiment(p.zal.WebEditorsFeatures, 'on'),
                    X = null == f || null == (e = f.trailer) ? void 0 : e.isAvailable,
                    G = k.checkExperiment(p.zal.WebNextArtistInfo, 'on');
                return (0, o.jsxs)(d.W1, {
                    isMobile: T,
                    offsetOptions: 10,
                    open: g,
                    onOpenChange: A,
                    ariaLabel: E({ id: 'interface-actions.context-menu' }),
                    containerProps: (0, n.Am)(n.Kq.artist.ARTIST_CONTEXT_MENU),
                    ...x,
                    children: [
                        q && (0, o.jsx)(v.aQ, { fallback: (0, o.jsx)(m.dx, { entityVariant: p.DwC.ARTIST, adminUrl: B }) }),
                        !T && (0, o.jsx)(v.aQ, { fallback: (0, o.jsx)(v.L1, { onClick: N, isPinned: null == f ? void 0 : f.isPinned }) }),
                        (0, o.jsx)(v.aQ, {
                            fallback: (0, o.jsx)(v.TW, {
                                onClick: M,
                                isLiked: null == f ? void 0 : f.isLiked,
                                disabled: !j.isAuthorized || !(null == f ? void 0 : f.isAvailable),
                            }),
                        }),
                        X && (0, o.jsx)(v.aQ, { fallback: (0, o.jsx)(v.No, { onClick: U }) }),
                        (0, o.jsx)(v.aQ, {
                            fallback: (0, o.jsx)(v.C6, { onClick: F, disabled: !(null == f ? void 0 : f.isAvailable), variant: p.IGO.ARTIST, onOpenMenuChange: A }),
                        }),
                        (0, o.jsx)(v.Ht, { disabled: !f, shareLink: R, entityMeta: W }),
                        G &&
                            (0, o.jsx)(v.aQ, {
                                fallback: (0, o.jsx)(d.Dr, {
                                    onClick: H,
                                    icon: (0, o.jsx)(u.Icon, { variant: 'info', size: 'xxs' }),
                                    ...(0, n.Am)(n.Kq.artist.ARTIST_CONTEXT_MENU_ABOUT_ARTIST_BUTTON),
                                    children: z,
                                }),
                            }),
                        (0, o.jsx)(v.aQ, {
                            fallback: (0, o.jsx)(v.DZ, { onClick: w, isDisliked: null == f ? void 0 : f.isDisliked, disabled: !(null == f ? void 0 : f.isAvailable) }),
                        }),
                    ],
                });
            });
        },
        25122: (t) => {
            t.exports = {
                root: 'ArtistAboutModalContent_root__XGW1F',
                header: 'ArtistAboutModalContent_header__ws7Ap',
                title: 'ArtistAboutModalContent_title__hMO2k',
                subtitle: 'ArtistAboutModalContent_subtitle__OpssN',
                descriptionWrapper: 'ArtistAboutModalContent_descriptionWrapper__jNL4G',
                description: 'ArtistAboutModalContent_description__KlWvL',
                readMoreButton: 'ArtistAboutModalContent_readMoreButton__1ageU',
                bandlinkScanner: 'ArtistAboutModalContent_bandlinkScanner__Fwv2x',
            };
        },
        34698: (t) => {
            t.exports = {
                root: 'ArtistAboutModalShimmer_root__RWCDi',
                entityName: 'ArtistAboutModalShimmer_entityName__eBJym',
                title: 'ArtistAboutModalShimmer_title__0uj5d',
            };
        },
        37215: (t, e, r) => {
            'use strict';
            var o;
            r.d(e, { g: () => o }),
                (function (t) {
                    (t.RATING = 'rating'), (t.YEAR = 'year');
                })(o || (o = {}));
        },
        40996: (t, e, r) => {
            'use strict';
            r.d(e, { L: () => u });
            var o,
                i = r(97020),
                a = r(55178),
                l = {
                    6699: (t, e, r) => {
                        Object.defineProperty(e, '__esModule', { value: !0 }), (e.useDebouncedToggle = void 0);
                        let o = r(352),
                            i = r(810);
                        e.useDebouncedToggle = (t) => {
                            let { delay: e, initialState: r, throttleTimeout: a } = t,
                                l = (0, i.useRef)(null),
                                [n, s] = (0, i.useState)(!!r),
                                c = (0, i.useMemo)(
                                    () =>
                                        (0, o.throttle)(() => {
                                            s(!r),
                                                l.current && window.clearTimeout(l.current),
                                                (l.current = window.setTimeout(() => {
                                                    s(!!r);
                                                }, e));
                                        }, a),
                                    [e, r, a],
                                ),
                                u = (0, i.useCallback)(() => {
                                    s(!!r), l.current && window.clearTimeout(l.current);
                                }, [r]);
                            return (
                                (0, i.useEffect)(
                                    () => () => {
                                        l.current && window.clearTimeout(l.current);
                                    },
                                    [],
                                ),
                                { state: n, handleDebouncedToggle: c, reset: u }
                            );
                        };
                    },
                    361: (t, e) => {
                        Object.defineProperty(e, '__esModule', { value: !0 }),
                            (e.getElementFromRefOrElement = void 0),
                            (e.getElementFromRefOrElement = (t) => {
                                if (void 0 !== t) {
                                    if (null === t || t instanceof HTMLElement) return t;
                                    if (null === t.current || t.current instanceof HTMLElement) return t.current;
                                }
                            });
                    },
                    352: (t) => {
                        t.exports = i;
                    },
                    810: (t) => {
                        t.exports = o || (o = r.t(a, 2));
                    },
                },
                n = {};
            function s(t) {
                var e = n[t];
                if (void 0 !== e) return e.exports;
                var r = (n[t] = { exports: {} });
                return l[t](r, r.exports, s), r.exports;
            }
            var c = {};
            (() => {
                Object.defineProperty(c, '__esModule', { value: !0 }), (c.useScroll = void 0);
                let t = s(810),
                    e = s(361),
                    r = s(6699);
                c.useScroll = (o) => {
                    let { onScroll: i, listenIsScrolling: a, elementRef: l } = o,
                        { state: n, handleDebouncedToggle: s } = (0, r.useDebouncedToggle)({ delay: 1e3, throttleTimeout: 100 }),
                        c = (0, t.useCallback)(() => {
                            a && s(), null == i || i();
                        }, [a, s, i]);
                    return (
                        (0, t.useEffect)(() => {
                            let t = (0, e.getElementFromRefOrElement)(l);
                            if (null === t) return;
                            let r = null != t ? t : window,
                                o = { capture: !0, passive: !0 };
                            return r.addEventListener('scroll', c, o), () => r.removeEventListener('scroll', c, o);
                        }, [l, c]),
                        n
                    );
                };
            })(),
                c.__esModule;
            var u = c.useScroll;
        },
        42048: (t) => {
            t.exports = {
                root: 'ArtistCard_root__x67BK',
                controls: 'ArtistCard_controls__jsqqI',
                cover: 'ArtistCard_cover__29ShU',
                coverBlock: 'ArtistCard_coverBlock__dBL4x',
                image: 'ArtistCard_image__pONJx',
                titleLink: 'ArtistCard_titleLink__G8Puz',
                playButton: 'ArtistCard_playButton__XZoTr',
                likeButton: 'ArtistCard_likeButton__LU9TL',
                menuButton: 'ArtistCard_menuButton__EynXG',
                pinButton: 'ArtistCard_pinButton__G_VOi',
                trailerButton: 'ArtistCard_trailerButton__a2NHm',
                control: 'ArtistCard_control___qv5j',
            };
        },
        42383: (t, e, r) => {
            'use strict';
            r.d(e, { HorizontalArtistCard: () => b });
            var o = r(32290),
                i = r(63618),
                a = r(96103),
                l = r(55178),
                n = r(60900),
                s = r(16172),
                c = r(59824),
                u = r(71483),
                d = r(91027),
                _ = r(58359),
                m = r(71926),
                p = r(33797),
                v = r(49574),
                f = r(44574),
                A = r(82586),
                g = r(17596),
                x = r.n(g);
            let h = (0, a.PA)((t) => {
                    var e;
                    let { className: r, artist: a, likeIconSize: n = 'xxs' } = t,
                        { user: c, trailer: u } = (0, v.Pjs)(),
                        _ = (0, v.PT7)(),
                        { sendLikeSearchFeedback: m } = (0, v.zEv)(),
                        g = (0, p.KX)(a),
                        [h, b] = (0, l.useState)(!1),
                        T = (0, d.c)(async () => {
                            h || a.isLiked || (b(!0), null == m || m()), await g();
                        }),
                        C = (0, v.Ftl)(),
                        y = (0, d.c)((t) => {
                            if ((t.stopPropagation(), _())) return void t.preventDefault();
                            u.openArtistTrailer(a.id), C(s.ky.Artist, a.id);
                        });
                    return (0, o.jsxs)('div', {
                        className: (0, i.$)(x().root, x().controls, r, { [x().controls_disabled]: !a.isAvailable }),
                        children: [
                            a.isAvailable &&
                                (0, o.jsxs)(o.Fragment, {
                                    children: [
                                        (0, o.jsx)(f.aQ, {
                                            fallback: (0, o.jsx)(f.cy, {
                                                size: 'xs',
                                                iconSize: n,
                                                className: (0, i.$)(x().item, x().likeIcon),
                                                isLiked: a.isLiked,
                                                onClick: T,
                                                disabled: !c.isAuthorized,
                                            }),
                                        }),
                                        (null == (e = a.trailer) ? void 0 : e.isAvailable) &&
                                            (0, o.jsx)(f.aQ, {
                                                fallback: (0, o.jsx)(f.kJ, {
                                                    className: (0, i.$)(x().item, x().trailerIcon),
                                                    iconSize: 'xs',
                                                    variant: 'text',
                                                    onClick: y,
                                                    withRipple: !1,
                                                }),
                                            }),
                                    ],
                                }),
                            (0, o.jsx)('div', {
                                className: (0, i.$)(x().item, x().item_buttonArrow),
                                children: (0, o.jsx)(A.Icon, { className: f.$f.buttonArrow, variant: 'arrowRight', size: 'xs' }),
                            }),
                        ],
                    });
                }),
                b = (0, a.PA)((t) => {
                    let { className: e, artist: r, pageId: a, description: A, coverClassName: g, playButtonIconSize: x, likeIconSize: b } = t,
                        { formatMessage: T } = (0, n.A)(),
                        { ref: C, intersectionPropertyId: y } = (0, v.nMI)(),
                        { from: j } = (0, v.fyy)({ pageId: a }),
                        [k, N] = (0, l.useState)(!1),
                        M = (0, v.ZpR)(r.url),
                        w = (0, v.brA)(),
                        I = (0, v.PT7)(),
                        { sendNavigateSearchFeedback: S, sendPlaySearchFeedback: E } = (0, v.zEv)(),
                        L = (0, v.NKK)(),
                        {
                            paywall: { modal: R },
                        } = (0, v.Pjs)(),
                        O = (0, v.NPu)(),
                        {
                            isPlaying: B,
                            isCurrent: P,
                            togglePlay: K,
                        } = (0, v.Dx4)({ playContextParams: { contextData: { type: u.K.Artist, meta: { id: Number(r.id) }, from: j }, loadContextMeta: !0 } }),
                        D = (0, p.SA)({ artist: r, callback: M }),
                        z = (0, p.SA)({ artist: r, callback: K }),
                        F = (0, d.c)((t) => {
                            null == S || S(), L({ to: s.QT.ArtistScreen }), D(t);
                        }),
                        U = (0, d.c)(() => {
                            if (!I()) {
                                if (O) return void R.open();
                                k || B || (N(!0), null == E || E()), z(), w(!B);
                            }
                        }),
                        H = (0, d.c)((t) => {
                            (0, _.P)(t, f.$f.ripple), F(t);
                        }),
                        W = (0, d.c)((t) => {
                            t.stopPropagation(), F(t);
                        }),
                        q = (0, l.useCallback)(
                            (t) =>
                                (0, o.jsx)(f.q1, {
                                    isAvailable: r.isAvailable,
                                    isDisliked: !1,
                                    coverUri: r.coverUri,
                                    title: r.name,
                                    className: (0, i.$)(f.$f.playButtonCell, g),
                                    radius: 'round',
                                    alt: T({ id: 'entity-names.artist-name' }, { artistName: r.name }),
                                    ...t,
                                }),
                            [r, g, T],
                        ),
                        X = null == q ? void 0 : q({ onPlayButtonClick: U, isPlaying: B, isCurrent: P, playButtonIconSize: x }),
                        G = (0, l.useMemo)(
                            () =>
                                r.url && r.isAvailable
                                    ? (0, o.jsx)(f.N_, {
                                          className: (0, i.$)(f.$f.text, f.$f.titleLink),
                                          href: r.url,
                                          onClick: W,
                                          ...(0, c.Am)(c.Kq.artist.SEARCH_ARTIST_CARD_TITLE_LINK),
                                          children: r.name,
                                      })
                                    : (0, o.jsx)(m.Caption, {
                                          className: (0, i.$)(f.$f.text, f.$f.titleText),
                                          size: 'm',
                                          variant: 'div',
                                          type: 'text',
                                          children: r.name,
                                      }),
                            [r.isAvailable, r.name, r.url, W],
                        );
                    return (0, o.jsxs)(f.Cj, {
                        ref: C,
                        'data-intersection-property-id': y,
                        className: (0, i.$)(f.$f.root, { [f.$f.root_disabled]: !r.isAvailable }, e),
                        'aria-label': r.name,
                        onClick: H,
                        ...(0, c.Am)(c.Kq.artist.HORIZONTAL_ARTIST_CARD),
                        children: [
                            X,
                            (0, o.jsx)(f.ro, {
                                isDisabled: !r.isAvailable,
                                isDisliked: !1,
                                likesCount: r.actualLikesCount,
                                isLiked: r.isLiked,
                                title: G,
                                description: A,
                            }),
                            (0, o.jsx)(h, { className: f.$f.controlsBar, artist: r, likeIconSize: b }),
                        ],
                    });
                });
        },
        48104: (t, e, r) => {
            'use strict';
            r.d(e, { ArtistAboutModal: () => L });
            var o = r(32290),
                i = r(96103),
                a = r(55178),
                l = r(60900),
                n = r(59824),
                s = r(6752),
                c = r(63423),
                u = r(82586),
                d = r(84638),
                _ = r(49574),
                m = r(44574),
                p = r(6820),
                v = r.n(p),
                f = r(44989),
                A = r(8092),
                g = r(71926),
                x = r(33797),
                h = r(39407),
                b = r(91027),
                T = r(50162),
                C = r(89214),
                y = r.n(C);
            let j = (0, i.PA)((t) => {
                let { covers: e } = t,
                    { formatMessage: r } = (0, l.A)(),
                    {
                        modals: { imageSliderModal: i, artistAboutModal: a },
                    } = (0, _.Pjs)(),
                    s = e.slice(0, 1),
                    u = e.length - 2 + 1,
                    d = (0, b.c)((t) => () => {
                        a.close(), i.openImages({ images: e, initialSlideIndex: t });
                    });
                return (0, o.jsxs)('div', {
                    className: y().root,
                    ...(0, n.Am)(n.e8.content.ARTIST_ABOUT_IMAGE_SLIDER),
                    children: [
                        s.map((t, e) =>
                            (0, o.jsx)(
                                c.$,
                                {
                                    className: y().button,
                                    onClick: d(e),
                                    'aria-label': r({ id: 'slider.view-artist-covers' }),
                                    ...(0, n.Am)(n.e8.content.ARTIST_ABOUT_IMAGE_SLIDER_BUTTON),
                                    children: (0, o.jsx)(T.Image, { fit: 'contain', className: y().image, src: t, size: 400, withAvatarReplace: !0 }),
                                },
                                e,
                            ),
                        ),
                        e[1] &&
                            (0, o.jsxs)(c.$, {
                                className: y().button,
                                onClick: d(1),
                                'aria-label': r({ id: 'slider.view-artist-covers' }),
                                ...(0, n.Am)(n.e8.content.ARTIST_ABOUT_IMAGE_SLIDER_BUTTON),
                                children: [
                                    (0, o.jsx)(T.Image, { fit: 'contain', className: y().image, src: e[1], size: 400, withAvatarReplace: !0 }),
                                    (0, o.jsx)('div', {
                                        className: y().moreCovers,
                                        children: (0, o.jsx)(g.Caption, {
                                            variant: 'span',
                                            className: y().moreCoversText,
                                            size: 'm',
                                            weight: 'medium',
                                            children: (0, o.jsx)(h.A, { id: 'slider.images-left-count', values: { imagesLeft: u } }),
                                        }),
                                    }),
                                ],
                            }),
                    ],
                });
            });
            var k = r(25122),
                N = r.n(k);
            let M = (0, i.PA)(() => {
                var t, e, r, i, a, s, c;
                let { formatMessage: u } = (0, l.A)(),
                    {
                        modals: { artistAboutModal: d },
                    } = (0, _.Pjs)(),
                    { state: m, setState: p } = (0, f.e)(!1),
                    v = (0, x.eI)(d.artistType);
                return (0, o.jsxs)('div', {
                    className: N().root,
                    children: [
                        d.isResolved &&
                            (0, o.jsxs)('header', {
                                className: N().header,
                                ...(0, n.Am)(n.e8.content.ARTIST_ABOUT_HEADER),
                                children: [
                                    (0, o.jsx)(g.Caption, {
                                        variant: 'div',
                                        type: 'text',
                                        size: 'm',
                                        weight: 'medium',
                                        className: N().subtitle,
                                        ...(0, n.Am)(n.e8.content.ARTIST_ABOUT_HEADER_SUBTITLE),
                                        children: v,
                                    }),
                                    (0, o.jsx)(g.Heading, {
                                        size: 'xxl',
                                        variant: 'div',
                                        className: N().title,
                                        ...(0, n.Am)(n.e8.content.ARTIST_ABOUT_HEADER_TITLE),
                                        children: null == (t = d.artist) ? void 0 : t.name,
                                    }),
                                ],
                            }),
                        d.description &&
                            (0, o.jsx)(A.K, {
                                moreText: u({ id: 'track-modal.read-more' }),
                                className: N().descriptionWrapper,
                                buttonClassName: N().readMoreButton,
                                buttonProps: (0, n.Am)(n.e8.content.ARTIST_ABOUT_DESCRIPTION_READ_MORE_BUTTON),
                                open: m,
                                onOpenChange: p,
                                lineClamp: 7,
                                withFade: !0,
                                ...(0, n.Am)(n.e8.content.ARTIST_ABOUT_DESCRIPTION),
                                children: (0, o.jsx)(g.Caption, {
                                    variant: 'div',
                                    className: N().description,
                                    size: 'l',
                                    weight: 'medium',
                                    ...(0, n.Am)(n.e8.content.ARTIST_ABOUT_DESCRIPTION_TEXT),
                                    children: d.description,
                                }),
                            }),
                        d.isArtistStatsAvailable &&
                            (0, o.jsx)(x.FO, { lastMonthListeners: null != (s = d.lastMonthListeners) ? s : 0, lastMonthListenersDelta: d.lastMonthListenersDelta }),
                        d.isResolved &&
                            (0, o.jsx)(x.lW, {
                                className: N().bandlinkScanner,
                                artistId: null == (e = d.artist) ? void 0 : e.id,
                                artistName: null == (r = d.artist) ? void 0 : r.name,
                            }),
                        d.links &&
                            (null == (i = d.links) ? void 0 : i.length) > 0 &&
                            (0, o.jsx)(x.KH, { links: d.links, artistName: null != (c = null == (a = d.artist) ? void 0 : a.name) ? c : '' }),
                        d.covers && d.covers.length > 0 && (0, o.jsx)(j, { covers: d.covers }),
                    ],
                });
            });
            var w = r(79856),
                I = r(34698),
                S = r.n(I);
            let E = () =>
                    (0, o.jsxs)('div', {
                        className: S().root,
                        children: [(0, o.jsx)(w.Shimmer, { className: S().entityName }), (0, o.jsx)(w.Shimmer, { className: S().title }), (0, o.jsx)(m.q$, { count: 4 })],
                    }),
                L = (0, i.PA)(() => {
                    let { formatMessage: t } = (0, l.A)(),
                        { notify: e } = (0, _.lkh)(),
                        { contentRef: r } = (0, _.gKY)(),
                        {
                            modals: { artistAboutModal: i },
                            settings: { isMobile: p },
                        } = (0, _.Pjs)();
                    (0, a.useEffect)(() => {
                        i.isRejected &&
                            (e((0, o.jsx)(m.hT, { error: t({ id: 'artist-errors.error-during-loading-artist-info' }) }), { containerId: _.uQT.ERROR }), i.close());
                    }, [i, i.isRejected, t, e]);
                    let f = (0, s.L)(() => (i.isLoading ? (0, o.jsx)(E, {}) : (0, o.jsx)(M, {})));
                    return (0, o.jsxs)(d.a, {
                        placement: p ? 'default' : 'right',
                        size: p ? 'fullscreen' : 'fitContent',
                        open: i.modal.isOpened,
                        onClose: i.close,
                        contentClassName: v().modalContent,
                        headerClassName: v().header,
                        className: v().root,
                        overlayClassName: v().overlay,
                        onOpenChange: i.onOpenChange,
                        labelClose: t({ id: 'interface-actions.close' }),
                        portalNode: p ? null : r,
                        showHeader: !1,
                        containerProps: (0, n.Am)(n.Xk.artist.ARTIST_ABOUT_MODAL),
                        children: [
                            (0, o.jsx)(c.$, {
                                radius: 'round',
                                color: 'secondary',
                                size: 'xxs',
                                icon: (0, o.jsx)(u.Icon, { variant: 'close', size: 'xxs' }),
                                className: v().closeButton,
                                onClick: i.close,
                                'aria-label': t({ id: 'interface-actions.close' }),
                                ...(0, n.Am)(n.Xk.artist.ARTIST_ABOUT_MODAL_CLOSE_BUTTON),
                            }),
                            f,
                        ],
                    });
                });
        },
        51496: (t, e, r) => {
            'use strict';
            var o;
            r.d(e, { U: () => o }),
                (function (t) {
                    (t.UNKNOWN = 'unknown'), (t.MALE = 'male'), (t.FEMALE = 'female');
                })(o || (o = {}));
        },
        58634: (t, e, r) => {
            'use strict';
            r.d(e, {
                r3: () => s,
                EV: () => c,
                y8: () => b.ArtistAboutModal,
                Xj: () => A,
                ao: () => T.ArtistCard,
                gg: () => C.ArtistContextMenu,
                G4: () => x,
                NN: () => y.ArtistTopCard,
                GV: () => h,
                cz: () => j.HorizontalArtistCard,
                LH: () => i,
                MX: () => n,
            });
            var o = r(60754);
            let i = (t) => (0, o.wg)({ title: t.title, url: t.action.weblink });
            var a = r(33797),
                l = r(62400);
            let n = (t) => {
                    let e = (0, a.as)({ artist: t.artist }),
                        r = (0, l.ws)(t.top);
                    return (0, o.wg)({ artist: e, listenTimeSeconds: t.listenTimeSeconds, top: r });
                },
                s = 20,
                c = 20;
            var u = r(21916),
                d = r(55178),
                _ = r(37215),
                m = r(49574);
            (0, d.cache)(async function (t, e) {
                let r,
                    o = arguments.length > 2 && void 0 !== arguments[2] ? arguments[2] : { artistId: t, page: 0, pageSize: s, sort: { sortBy: _.g.YEAR } };
                (0, m.LLJ)(t) || (0, u.notFound)();
                try {
                    let i,
                        { container: a, backendHostTld: l, locale: n, host: s, fullUrl: c, isRSC: u, requestUrl: d } = await (0, m.IhE)();
                    if (u) return { artist: void 0, albums: void 0, backendHostTld: l, locale: n, host: s, fullUrl: c, requestUrl: d };
                    let _ = a.get(m.O9U),
                        p = await _.getInfo({ artistId: t });
                    switch (e) {
                        case m.hVl.COMPILATIONS:
                            i = await _.getAlsoAlbums(o);
                            break;
                        case m.hVl.DISCOGRAPHY:
                            i = await _.getDiscographyAlbums(o);
                            break;
                        default:
                            i = await _.getDirectAlbums(o);
                    }
                    r = { artist: p, albums: i, backendHostTld: l, locale: n, host: s, fullUrl: c, requestUrl: d };
                } catch (t) {
                    return (0, m.tfF)(t, () => ({ artist: void 0, albums: void 0, ...(0, m.Bj3)() }));
                }
                let { artist: i } = r;
                return i || (0, u.notFound)(), r;
            }),
                (0, d.cache)(async function (t) {
                    let e,
                        r = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : { artistId: t, page: 0, pageSize: c };
                    try {
                        let { container: o, backendHostTld: i, locale: a, host: l, fullUrl: n, isRSC: s, requestUrl: c } = await (0, m.IhE)();
                        if (s) return { artist: void 0, clips: void 0, backendHostTld: i, locale: a, host: l, fullUrl: n, requestUrl: c };
                        let u = o.get(m.O9U),
                            d = await u.getInfo({ artistId: t }),
                            _ = await u.getClips(r);
                        e = { artist: d, clips: _, backendHostTld: i, locale: a, host: l, fullUrl: n, requestUrl: c };
                    } catch (t) {
                        return (0, m.tfF)(t, () => ({ artist: void 0, clips: void 0, ...(0, m.Bj3)() }));
                    }
                    let { artist: o } = e;
                    return o || (0, u.notFound)(), e;
                });
            let p = (0, d.cache)(async () => {
                var t;
                let { container: e, isRSC: r } = await (0, m.IhE)();
                if (r) return;
                let o = await (0, m.hlb)(),
                    i = null == o ? void 0 : o.experimentsData;
                if (!i) return;
                let a = e.get(m.ooW);
                if ((null == i || null == (t = i.WebNextConcertsLocationAll) ? void 0 : t.group) !== 'on') return;
                let l = a.get(m.cYZ.ConcertLocation);
                if ('number' == typeof l) return [l];
            });
            (0, d.cache)(async (t) => {
                let e;
                (0, m.LLJ)(t) || (0, u.notFound)();
                try {
                    let { container: r, backendHostTld: o, locale: i, host: a, fullUrl: l, isRSC: n, requestUrl: s } = await (0, m.IhE)();
                    if (n) return { artist: void 0, backendHostTld: o, locale: i, host: a, fullUrl: l, requestUrl: s };
                    let c = r.get(m.O9U),
                        [u, d] = await Promise.all([c.getInfo({ artistId: t }), p()]),
                        _ = await c.getConcerts({ artistId: Number(t), locations: d });
                    e = { artist: u, concerts: _, backendHostTld: o, locale: i, host: a, fullUrl: l, requestUrl: s };
                } catch (t) {
                    return (0, m.tfF)(t, () => ({ artist: void 0, concerts: void 0, ...(0, m.Bj3)() }));
                }
                let { artist: r } = e;
                return r || (0, u.notFound)(), e;
            }),
                (0, d.cache)(async (t) => {
                    let e;
                    (0, m.LLJ)(t) || (0, u.notFound)();
                    try {
                        let { container: r, backendHostTld: o, locale: i, host: a, fullUrl: l, isRSC: n, requestUrl: s } = await (0, m.IhE)();
                        if (n) return { artist: void 0, familiar: void 0, backendHostTld: o, locale: i, host: a, fullUrl: l, requestUrl: s };
                        let c = r.get(m.O9U),
                            u = await c.getInfo({ artistId: t }),
                            d = await c.getFamiliarYou({ artistId: t, waveTracksLimit: 100, collectionTracksLimit: 100, collectionAlbumsLimit: 20 });
                        e = { artist: u, familiar: d, backendHostTld: o, locale: i, host: a, fullUrl: l, requestUrl: s };
                    } catch (t) {
                        return (0, m.tfF)(t, () => ({ artist: void 0, familiar: void 0, ...(0, m.Bj3)() }));
                    }
                    let { artist: r } = e;
                    return r || (0, u.notFound)(), e;
                }),
                (0, d.cache)(async (t) => {
                    let e;
                    (0, m.LLJ)(t) || (0, u.notFound)();
                    try {
                        let { container: r, backendHostTld: o, locale: i, host: a, fullUrl: l, isRSC: n, requestUrl: s } = await (0, m.IhE)();
                        if (n) return { artist: void 0, backendHostTld: o, locale: i, host: a, fullUrl: l, requestUrl: s };
                        let c = r.get(m.O9U);
                        e = { artist: await c.getInfo({ artistId: t }), backendHostTld: o, locale: i, host: a, fullUrl: l, requestUrl: s };
                    } catch (t) {
                        return (0, m.tfF)(t, () => ({ artist: void 0, ...(0, m.Bj3)() }));
                    }
                    let { artist: r } = e;
                    return r || (0, u.notFound)(), e;
                }),
                (0, d.cache)(async (t) => {
                    let e;
                    (0, m.LLJ)(t) || (0, u.notFound)();
                    try {
                        let { container: r, backendHostTld: o, locale: i, host: a, fullUrl: l, isRSC: n, requestUrl: s } = await (0, m.IhE)();
                        if (n) return { artist: void 0, similarArtists: void 0, backendHostTld: o, locale: i, host: a, fullUrl: l, requestUrl: s };
                        let c = r.get(m.O9U),
                            u = await c.getInfo({ artistId: t }),
                            d = await c.getSimilarArtists({ artistId: t });
                        e = { artist: u, similarArtists: d, backendHostTld: o, locale: i, host: a, fullUrl: l, requestUrl: s };
                    } catch (t) {
                        return (0, m.tfF)(t, () => ({ artist: void 0, similarArtists: void 0, ...(0, m.Bj3)() }));
                    }
                    let { artist: r } = e;
                    return r || (0, u.notFound)(), e;
                });
            var v = r(421),
                f = r(95428);
            let A = o.gK
                    .compose(
                        o.gK.model('ArtistAboutModal', {
                            modal: f.qt,
                            artist: o.gK.maybeNull(a.PK),
                            artistType: o.gK.maybeNull(o.gK.enumeration(Object.values(v.o))),
                            description: o.gK.maybeNull(o.gK.string),
                            lastMonthListeners: o.gK.maybeNull(o.gK.number),
                            lastMonthListenersDelta: o.gK.optional(o.gK.number, 0),
                            covers: o.gK.maybeNull(o.gK.array(o.gK.string)),
                            links: o.gK.maybeNull(o.gK.array(a.dP)),
                        }),
                        f.XT,
                        f.pl,
                    )
                    .views((t) => ({
                        get isArtistStatsAvailable() {
                            return Number.isFinite(t.lastMonthListeners);
                        },
                    }))
                    .actions((t) => {
                        let e = {
                            open(r) {
                                r && (e.getData(r), t.modal.open());
                            },
                            getData: (0, o.L3)(function* (e) {
                                let { artistsResource: r, modelActionsLogger: i } = (0, o._$)(t);
                                if (!t.isLoading)
                                    try {
                                        var l, n, s, c, u;
                                        t.loadingState = m.GuX.PENDING;
                                        let i = yield r.getAboutArtist({ artistId: e });
                                        (t.artist = (0, a.as)({ artist: i.artist })),
                                            (t.description = null != (u = i.description) ? u : null),
                                            (t.artistType = i.artistType === v.o.COMPOSER ? v.o.COMPOSER : v.o.ARTIST),
                                            i.stats &&
                                                Number.isFinite(null == (l = i.stats) ? void 0 : l.lastMonthListeners) &&
                                                (t.lastMonthListeners = i.stats.lastMonthListeners),
                                            i.stats &&
                                                Number.isFinite(null == (n = i.stats) ? void 0 : n.lastMonthListenersDelta) &&
                                                (t.lastMonthListenersDelta = i.stats.lastMonthListenersDelta);
                                        let d = null == (s = i.covers) ? void 0 : s.map((t) => t.uri).filter((t) => !!t);
                                        d && (t.covers = (0, o.wg)(d)),
                                            (t.links = (0, o.wg)(null == (c = i.links) ? void 0 : c.map(a.DZ))),
                                            t.loadingState !== m.GuX.IDLE && (t.loadingState = m.GuX.RESOLVE);
                                    } catch (e) {
                                        i.error(e), t.loadingState !== m.GuX.IDLE && (t.loadingState = m.GuX.REJECT);
                                    }
                            }),
                            close() {
                                t.modal.close(), e.reset();
                            },
                            onOpenChange(r) {
                                t.modal.onOpenChange(r), r || e.reset();
                            },
                            reset() {
                                (t.loadingState = m.GuX.IDLE),
                                    (t.description = null),
                                    (t.lastMonthListeners = null),
                                    (t.lastMonthListenersDelta = 0),
                                    (t.artistType = null),
                                    t.destroyItems([t.artist, t.covers, t.links]);
                            },
                        };
                        return e;
                    }),
                g = o.gK.model('ArtistBrandedButton', { title: o.gK.maybeNull(o.gK.string), url: o.gK.maybeNull(o.gK.string) }),
                x = o.gK
                    .model('ArtistMeta', {
                        artist: a.PK,
                        donationUrl: o.gK.maybe(o.gK.string),
                        lastMonthListeners: o.gK.maybe(o.gK.number),
                        brandedButton: o.gK.maybe(g),
                        covers: o.gK.maybeNull(o.gK.array(o.gK.string)),
                    })
                    .views((t) => ({
                        get hasCovers() {
                            var e;
                            return !!(null == (e = t.covers) ? void 0 : e.length);
                        },
                    })),
                h = o.gK
                    .model('ArtistTop', { artist: a.PK, listenTimeSeconds: o.gK.number, top: o.gK.maybe(l.Iz) })
                    .views((t) => ({
                        get isAvailableForPlaying() {
                            if (void 0 === t.artist.counts) return !0;
                            return t.artist.counts.tracks > 0;
                        },
                    }))
                    .actions((t) => ({ getKey: (e) => ''.concat(e, '_').concat(t.artist.id) }));
            var b = r(48104),
                T = r(7474),
                C = r(21564),
                y = r(76238),
                j = r(42383);
        },
        75645: (t, e, r) => {
            'use strict';
            r.d(e, { hO: () => _, jr: () => m, jp: () => p, QZ: () => i }), r(85377);
            var o = r(55178);
            let i = (t, e, r) => {
                let i = null != r && r;
                return {
                    topColorStyle: (0, o.useMemo)(() => {
                        if (void 0 === e) return;
                        let r = i ? 93 : 17,
                            o = e - r;
                        return { '--average-color-background': t, transform: 'translateY('.concat(e >= r ? 0 : o, 'px)'), opacity: 1 };
                    }, [e, i, t]),
                    headerStyle: (0, o.useMemo)(() => ({ '--average-color-background': t }), [t]),
                };
            };
            var a = r(32290),
                l = r(8055),
                n = r(96103),
                s = r(40996),
                c = r(49574),
                u = r(44574);
            let d = (t) => {
                    let { element: e, scrollTop: r, isMobile: o } = t,
                        i = 0.6 * e.clientHeight;
                    return r + (o ? 60 : 76) >= e.offsetTop + i;
                },
                _ = (0, n.PA)((t) => {
                    let { children: e, scrollElement: r, outerTitle: i = '', headerElement: n, headerThreshold: _, shouldHideHeader: m } = t,
                        [p, v] = (0, o.useState)(i),
                        [f, A] = (0, o.useState)(null),
                        [g, x] = (0, o.useState)(null),
                        [h, b] = (0, o.useState)(null),
                        [T, C] = (0, o.useState)(!1),
                        [y, j] = (0, o.useState)(!1),
                        [k, N] = (0, o.useState)(!1),
                        [M, w] = (0, o.useState)(!1),
                        {
                            settings: { isMobile: I },
                        } = (0, c.Pjs)();
                    (0, o.useLayoutEffect)(() => {
                        v(i);
                    }, [i]);
                    let S = (0, o.useCallback)(() => {
                        let t = null != _ ? _ : 10,
                            e = n ? Number(n.offsetTop) - t : t;
                        e < 0 && (e = 0);
                        let o = (t) => {
                            N(t > e),
                                m && w(t > 30),
                                (null == f ? void 0 : f.current) && C(d({ element: null == f ? void 0 : f.current, scrollTop: t, isMobile: I })),
                                (null == g ? void 0 : g.current) && j(d({ element: null == g ? void 0 : g.current, scrollTop: t, isMobile: I }));
                        };
                        I ? o(window.scrollY) : r && o(r.scrollTop);
                    }, [_, n, I, m, f, g, r]);
                    (0, o.useEffect)(() => {
                        I ? 0 === window.scrollY && N(!1) : (r && 0 !== r.scrollTop) || N(!1);
                    }, [r, null == r ? void 0 : r.scrollTop, I]);
                    let E = (0, o.useMemo)(() => (I ? { onScroll: (0, l.A)(S, 200) } : { onScroll: (0, l.A)(S, 200), elementRef: r }), [S, r, I]);
                    (0, s.L)(E);
                    let L = (0, o.useMemo)(
                        () => ({
                            title: p,
                            setTitle: v,
                            titleElement: f,
                            scrollElement: I ? null : r,
                            setTitleElement: A,
                            childElement: g,
                            setChildElement: x,
                            child: h,
                            setChild: b,
                            isScrolledChild: y,
                            isScrolledTitle: T,
                            isScrolling: k,
                            isHeaderHidden: M,
                        }),
                        [p, T, f, r, k, I, h, g, y, M],
                    );
                    return (0, a.jsx)(u.B9.Provider, { value: L, children: e });
                }),
                m = (t) => {
                    let { children: e, child: r, className: i } = t,
                        { setChildElement: l, setChild: n } = (0, o.useContext)(u.B9),
                        s = (0, o.useRef)(null);
                    return (
                        (0, o.useEffect)(() => {
                            (null == s ? void 0 : s.current) && l(s), r && n(r);
                        }, [s, r, l, n]),
                        (0, o.useEffect)(
                            () => () => {
                                n(null);
                            },
                            [n],
                        ),
                        (0, a.jsx)('div', { ref: s, className: i, children: e })
                    );
                },
                p = (t) => {
                    let { children: e, title: r, className: i } = t,
                        { setTitleElement: l, setTitle: n } = (0, o.useContext)(u.B9),
                        s = (0, o.useRef)(null);
                    return (
                        (0, o.useEffect)(() => {
                            (null == s ? void 0 : s.current) && l(s), r && n(r);
                        }, [s, r, l, n]),
                        (0, o.useEffect)(
                            () => () => {
                                n('');
                            },
                            [n],
                        ),
                        (0, a.jsx)('div', { ref: s, className: i, children: e })
                    );
                };
        },
        76238: (t, e, r) => {
            'use strict';
            r.d(e, { ArtistTopCard: () => f });
            var o = r(32290),
                i = r(63618),
                a = r(96103),
                l = r(60900),
                n = r(13365),
                s = r(6752),
                c = r(82586),
                u = r(71926),
                d = r(33797),
                _ = r(7474),
                m = r(86384),
                p = r.n(m);
            let v = { [n._.UP]: 'chartUp', [n._.DOWN]: 'chartDown', [n._.SAME]: 'chartSame', [n._.NEW]: 'chartNew' },
                f = (0, a.PA)((t) => {
                    let { artist: e, position: r, progress: a, listenTimeSeconds: n, className: m } = t,
                        { formatMessage: f } = (0, l.A)(),
                        A = (0, d.Uw)(n, f),
                        g = a ? v[a] : null,
                        x = g
                            ? (0, o.jsx)(c.Icon, { variant: g, size: 'xxs', className: (0, i.$)(p().progressIcon, p()['progressIcon_'.concat(a)]), 'aria-hidden': !0 })
                            : null,
                        h = (0, s.L)(() =>
                            void 0 !== r
                                ? (0, o.jsxs)('div', {
                                      className: p().positionIndicator,
                                      children: [
                                          (0, o.jsx)(u.Caption, { variant: 'span', type: 'entity', size: 'm', weight: 'bold', children: r }),
                                          1 === r && (0, o.jsx)(c.Icon, { variant: 'crown', size: 'xxs', className: p().crownIcon, 'aria-hidden': 'true' }),
                                          1 !== r && x,
                                      ],
                                  })
                                : void 0,
                        ),
                        b = (0, s.L)(() =>
                            (0, o.jsx)(u.Caption, { variant: 'div', type: 'controls', size: 's', weight: 'medium', className: p().listenTime, children: A }),
                        );
                    return (0, o.jsx)(_.ArtistCard, { artist: e, className: m, topTitleElement: h, bottomTitleElement: b, contentLinesCount: 3 });
                });
        },
        86384: (t) => {
            t.exports = {
                positionIndicator: 'ArtistTopCard_positionIndicator__Bs_Ga',
                crownIcon: 'ArtistTopCard_crownIcon__yxAH3',
                progressIcon: 'ArtistTopCard_progressIcon__gHcbF',
                progressIcon_up: 'ArtistTopCard_progressIcon_up__PeHBx',
                progressIcon_down: 'ArtistTopCard_progressIcon_down__0PKeO',
                progressIcon_same: 'ArtistTopCard_progressIcon_same___SujQ',
                progressIcon_new: 'ArtistTopCard_progressIcon_new__w3wA6',
                listenTime: 'ArtistTopCard_listenTime__P1_jw',
            };
        },
        89214: (t) => {
            t.exports = {
                root: 'ArtistAboutModalImageSlider_root__L18Xb',
                button: 'ArtistAboutModalImageSlider_button__GPXyc',
                image: 'ArtistAboutModalImageSlider_image__3CTLr',
                moreCovers: 'ArtistAboutModalImageSlider_moreCovers__7oDPM',
                moreCoversText: 'ArtistAboutModalImageSlider_moreCoversText__W_P8L',
            };
        },
    },
]);
