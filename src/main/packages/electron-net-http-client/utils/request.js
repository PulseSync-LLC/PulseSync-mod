...
function request(url, fetch, options) {
    ...
    const retryPolicy = options.retryPolicy || defaultRetryPolicy_js_1.defaultRetryPolicy;
    const totalRequestsLimit = retryPolicy.totalRequestsLimit || defaultTotalRequestsLimit_js_1.defaultTotalRequestsLimit;
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            throw new FetchRequestError_js_1.FetchRequestError({
                ...
                status: response.status ?? ErrorStatusCode_js_1.ErrorStatusCode.NON_HTTP_ERROR,
                headers: response.headers,
            });
        } catch (err) {
            clearTimeout(abortTimeout);
            const error = err;
            const status = error?.status ?? ErrorStatusCode_js_1.ErrorStatusCode.NON_HTTP_ERROR;
            ...
            if (retryCount < retryAttempts.length && retryCount < totalRequestsLimit - 1) {
                ...
                requestWithTimeout(url, fetch, { ...options, retryCount: retryCount + 1 }, delay)
                    .then(resolve)
                    .catch(reject);
            } else {
                reject(
                    new FetchRequestError_js_1.FetchRequestError({
                        ...
                        status: error?.status ?? ErrorStatusCode_js_1.ErrorStatusCode.NON_HTTP_ERROR,
