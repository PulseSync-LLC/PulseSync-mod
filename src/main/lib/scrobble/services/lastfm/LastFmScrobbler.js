...
class LastFmScrobbler {
    ...
    async login() {
        ...
        const childWindow = new electron_1.BrowserWindow({
            ...
            webPreferences: {
                preload: node_path_1.join(__dirname, 'preload.js'),
                devTools: true,
            },
        });
    ...
    handleLikeState(playingState) {
        if (!store_js_1.getModSettings()?.scrobblers?.lastfm.autoLike || !this.nowPlayingLastFmTrack) return;
        if (!playingState?.actionsStore?.isLiked) return;
    ...
    maybeScrobbleCurrentTrack() {
        ...
        if (!this.isTrackEligibleForScrobble(this.currentTrack, totalPlayedTime)) return false;
...
LastFmScrobbler.MAX_SCROBBLE_TIME_MS = 420000; // 7 minutes
