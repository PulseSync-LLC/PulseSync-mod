...
async function drawThumbnail(width, height, previousTrack, currentTrack, nextTrack, isPlaying = true) {
    ...
    const currImg = await sharp(currentTrack)
        .resize(isPlaying ? height : smallSize, isPlaying ? height : smallSize, { fit: 'inside' })
        .toBuffer();
    const prevImg = previousTrack ? await sharp(previousTrack).resize(smallSize).toBuffer() : null;
    const nextImg = nextTrack ? await sharp(nextTrack).resize(smallSize).toBuffer() : null;
