...
export const tabsHandler: ControlHandler<TabsControl> = {
    parse(value) {
        if (typeof value.id !== 'string' || !/^[a-z0-9_-]{1,100}$/i.test(value.id) || typeof value.label !== 'string' || !value.label.trim() || value.label.length > 200)
            return;
        ...
        for (const item of value.items) {
            if (
                ...
                !/^[a-z0-9_-]{1,80}$/i.test(item.value) ||
