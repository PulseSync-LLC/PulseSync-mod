...
export function SettingSelect({ description, disabled = false, onChange, options, title, titleAction, value }: SettingSelectProps) {
    ...
    const updateMenuPosition = useCallback(() => {
        ...
        const openAbove = spaceBelow < Math.min(estimatedHeight, 240) && spaceAbove > spaceBelow;
        const basePosition = {
    ...
    return (
        <div className={styles.row} data-disabled={disabled} ref={rootRef}>
            <span className={styles.text}>
                ...
                {resolvedDescription ? <span className={styles.description}>{resolvedDescription}</span> : null}
            </span>
            <button
            ...
            {isOpen && menuPosition
                ? createPortal(
                      <div className={styles.options} ref={menuRef} role="listbox" aria-label={title} style={menuPosition}>
                          {options.map((option) => (
                              <button
                              ...
                              >
                                  ...
                                  {option.value === value ? (
                                      <svg className={styles.check} width="16" height="16" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
