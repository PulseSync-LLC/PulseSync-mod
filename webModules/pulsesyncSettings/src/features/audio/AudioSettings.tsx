...
const WASAPI_ENABLED_KEY = 'modSettings.nativeAudioOutput.enableWasapiExclusiveOutput';
const WASAPI_FULL_VOLUME_KEY = 'modSettings.nativeAudioOutput.forceWasapiExclusiveFullVolume';
...
function getMaximumWasapiFormat(device: WasapiDevice | null) {
    let formats = Array.isArray(device?.supportedFormats)
        ? device.supportedFormats.filter(
              (format) => format.float !== true && Number(format.sampleRate) > 0 && Number(format.bitsPerSample) > 0 && Number(format.channels) > 0,
          )
        : [];
        ...
        formats
            ...
            .sort(
                (first, second) =>
                    ...
                    Number(second.containerBitsPerSample ?? second.bitsPerSample) - Number(first.containerBitsPerSample ?? first.bitsPerSample),
            )[0] ?? null
    );
}
...
function supports24Bit48000(device: WasapiDevice) {
    return Boolean(
        device.supportedFormats?.some((format) => format.sampleRate === 48000 && format.bitsPerSample === 24 && format.channels === 2 && format.float !== true),
    );
}

function getWasapiDeviceDescription(device: WasapiDevice | null, isDefault = false) {
    if (!device) return 'Системное устройство вывода будет определено автоматически.';

    const format = getMaximumWasapiFormat(device);
    const formatDescription = format
        ? `До: ${format.bitsPerSample} бит${
              format.containerBitsPerSample && format.containerBitsPerSample !== format.bitsPerSample ? ` (контейнер ${format.containerBitsPerSample} бит)` : ''
...
export function AudioSettings({ api, onRestartRequired }: AudioSettingsProps) {
    const r128Normalization = useBooleanModSetting(api, R128_NORMALIZATION_KEY, true);
    ...
    const wasapiFullVolume = useBooleanModSetting(api, WASAPI_FULL_VOLUME_KEY, false);
    ...
    useEffect(() => {
        ...
        void Promise.all([api.getWasapiExclusiveStatus(), api.listWasapiExclusiveDevices(), api.getSelectedWasapiExclusiveDeviceId()])
            .then(([status, devices, selectedDeviceId]) => {
                if (!active) return;
                setWasapiStatus((status ?? {}) as WasapiStatus);
                setWasapiDevices(Array.isArray(devices) ? devices : []);
                setSelectedWasapiDeviceId(typeof selectedDeviceId === 'string' ? selectedDeviceId : '');
            })
            .catch((reason: unknown) => {
                if (!active) return;
                setWasapiStatus({ available: false, supported: false });
                setWasapiDevices([]);
                setWasapiError(reason instanceof Error ? reason.message : 'Не удалось получить настройки WASAPI Exclusive');
            });
    ...
    useEffect(() => {
        ...
        const refreshDefaultDevice = (event: Event) => {
            const changedAt = Number((event as CustomEvent<Record<string, unknown>>).detail?.defaultDeviceChangedAt);
            if (!Number.isFinite(changedAt) || changedAt === defaultDeviceChangedAt) return;
        ...
        return () => window.removeEventListener('pulse-sync-wasapi-exclusive-output-state-change', refreshDefaultDevice);
    }, [isWindows, loadWasapiDevices]);

    const activeWasapiDevices = wasapiDevices.filter((device) => device.state === 'active');
    const defaultWasapiDevice = activeWasapiDevices.find((device) => device.isDefault) ?? activeWasapiDevices.find((device) => device.isDefaultConsole) ?? null;
    ...
    const wasapiSupported = Boolean(isWindows && wasapiStatus?.available && wasapiStatus.supported);
            ...
            ? ([
                  ...
                  },
                  {
                      ...
                      props: {
                          ...
                          description: yaspTap.value ? wasapiAvailabilityDescription : 'Сначала включите YASP Tap.',
                          badge: { label: 'ALPHA', disabled: wasapiSectionDisabled },
                          checked: wasapiEnabled.value,
                          disabled: wasapiEnabled.isLoading || !api || wasapiSectionDisabled,
                  ...
                  },
                  {
                      ...
                      props: {
                          ...
                          disabled: wasapiFullVolume.isLoading || !api || wasapiSectionDisabled,
                          onChange: (value: boolean) => void wasapiFullVolume.update(value),
                      },
                  },
                  {
                      id: 'wasapi-device',
                      ...
                      props: {
                          ...
                          onChange: (value: string) => {
                              void api
                                  ...
                                  .catch((reason: unknown) => {
                                      setWasapiError(reason instanceof Error ? reason.message : 'Не удалось выбрать устройство WASAPI Exclusive');
                                  });
                          },
                      },
                  },
    ...
    const error = r128Normalization.error ?? surroundAudio.error ?? yaspTap.error ?? wasapiEnabled.error ?? wasapiFullVolume.error ?? wasapiError;

    return <SettingsSection title="Настройки аудио" items={items} error={error} />;
}
