...
export function GlobalShortcutsSettings({ api }: GlobalShortcutsSettingsProps) {
    ...
    useEffect(() => {
        ...
        void api
            ...
            .catch((reason: unknown) => {
                if (active) setError(reason instanceof Error ? reason.message : 'Не удалось прочитать хоткеи');
            })
            .finally(() => {
    ...
    const persist = useCallback(
        async (nextEnabled: boolean, nextItems: ShortcutDraft[]) => {
            ...
            setError(undefined);
            try {
                await api.setModSetting(GLOBAL_SHORTCUTS_KEY, serializeConfig(nextEnabled, nextItems));
            } catch (reason) {
                setError(reason instanceof Error ? reason.message : 'Не удалось сохранить хоткеи');
            }
        },
        [api],
    );
    ...
    const updateAccelerator = (shortcutId: string, accelerator: string) => {
        updateItems((current) => current.map((item) => (item.id === shortcutId ? { ...item, accelerator } : item)));
    };

    const updateAction = (shortcutId: string, commandId: string, action: string) => {
        updateItems((current) =>
            current.map((item) =>
                item.id !== shortcutId
                    ? item
                          ...
                          commands: item.commands.map((command) =>
                              command.id !== commandId
                                  ...
                                  : {
                                        ...command,
                                        action,
                                        value: actionNeedsValue(action) ? command.value || ACTION_DEFAULT_VALUES[action] || '' : '',
                                    },
                          ),
                      },
            ),
    ...
    const updateActionValue = (shortcutId: string, commandId: string, value: string) => {
        updateItems((current) =>
            current.map((item) =>
                item.id !== shortcutId
                    ? item
    ...
    return (
        <section className={styles.section}>
            ...
            <SettingsItem id={GLOBAL_SHORTCUTS_KEY} searchText={['Глобальные горячие клавиши', 'Сочетания клавиш', 'Хоткеи', 'Добавить хоткей', 'Действие', 'Команда']}>
                <SettingToggle
            ...
            <div className={styles.toolbar}>
                <button className={styles.addButton} type="button" disabled={isLoading || !api} onClick={() => setItems((current) => [createShortcut(), ...current])}>
                    + Добавить хоткей
                </button>
            </div>
            <div className={styles.list}>
                {!items.length ? <div className={styles.empty}>Пока нет настроенных хоткеев.</div> : null}
                    ...
                    <article className={styles.card} key={item.id}>
                        <header className={styles.cardHeader}>
                            ...
                            <button
                                ...
                                type="button"
                                onClick={() => updateItems((current) => current.filter((shortcut) => shortcut.id !== item.id))}
                            >
                                Удалить хоткей
                            </button>
                        </header>
                            ...
                            return (
                                <div className={styles.command} key={command.id}>
                                    <SettingSelect
                                        ...
                                        disabled={!enabled}
                                        onChange={(action) => updateAction(item.id, command.id, action)}
                                    />
                                    {valueMeta ? (
                                        <SettingInput
                                            title="Значение"
                                            ...
                                            onChange={(value) => updateActionValue(item.id, command.id, value)}
                                        />
                                    ) : null}
                                                ...
                                                updateItems((current) =>
                                                    current.map((shortcut) =>
                                                        shortcut.id === item.id
                                                            ? {
                                                                  ...shortcut,
                                                                  commands: shortcut.commands.filter((currentCommand) => currentCommand.id !== command.id),
                                                              }
                                                            : shortcut,
                                                    ),
                                                )
