...
export function KeybindRecorder({ disabled = false, onChange, onRecordingChange, value }: KeybindRecorderProps) {
    ...
    useEffect(() => {
        ...
        const onKeyDown = (event: KeyboardEvent) => {
            ...
            const accelerator = [...(shiftedSymbol ? modifiers.filter((modifier) => modifier !== 'Shift') : modifiers), key].join('+');
    ...
    return (
        <div className={styles.row} ref={rootRef}>
            <span className={styles.text}>
                ...
                <span className={styles.description}>Нажмите справа и введите комбинацию. Esc отменяет запись, Backspace/Delete очищает поле.</span>
            </span>
            <button className={styles.recorder} data-recording={isRecording} type="button" disabled={disabled} onClick={() => setIsRecording((recording) => !recording)}>
