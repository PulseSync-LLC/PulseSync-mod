...
import { appSectionsSchema, appUpdatesSchema, downloaderSchema, playlistSchema } from './sections/general';
import { discordRpcSchema, lrclibSchema } from './sections/integrations';
import { miniplayerSchema, playerBarSchema, windowSchema } from './sections/interface';
import { systemSchema, ynisonSchema } from './sections/system';
import { vibeAppearanceSchema, vibeBehaviorSchema, vibePerformanceSchema } from './sections/vibe';
...
function schemaSection(id: string, label: string, schema: SettingsSectionSchema): SettingsSectionDefinition {
    return {
        id,
        label,
        render: ({ api, displayMaxFps, onRestartRequired }) => (
            <SchemaSettingsSection api={api} displayMaxFps={displayMaxFps} schema={schema} onRestartRequired={onRestartRequired} />
        ),
    };
}

function customSection(id: string, label: string, render: SettingsSectionDefinition['render']): SettingsSectionDefinition {
...
export const SETTINGS_GROUPS = [
    {
        ...
        sections: [
            ...
            customSection('shortcuts', 'Горячие клавиши', ({ api }) => <GlobalShortcutsSettings api={api} />),
        ],
    },
    {
        label: 'Интерфейс',
    ...
    },
    {
        label: 'Моя Волна',
        sections: [
            schemaSection('vibe-performance', 'Производительность', vibePerformanceSchema),
    ...
    },
    {
        ...
        sections: [
            ...
            customSection('lastfm', 'Скробблинг', ({ api }) => <LastFmSettings api={api} />),
        ],
    },
    {
        ...
        sections: [
            ...
            customSection('audio', 'Настройки аудио', ({ api, onRestartRequired }) => <AudioSettings api={api} onRestartRequired={onRestartRequired} />),
...
export const SETTINGS_SECTIONS: readonly SettingsSectionDefinition[] = SETTINGS_GROUPS.flatMap((group) => group.sections);
