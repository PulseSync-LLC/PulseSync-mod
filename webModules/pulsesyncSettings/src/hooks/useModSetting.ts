...
export function useModSetting<T extends ModSettingValue>(api: ModSettingsApi | undefined, key: string, fallback: T) {
    ...
    useEffect(() => {
        ...
        void api
            .getModSetting(key)
            .then((currentValue) => {
                if (active && !receivedUpdate) setValue(normalizeValue(currentValue, fallback));
            })
            .catch((reason: unknown) => {
                if (active) setError(reason instanceof Error ? reason.message : 'Не удалось прочитать настройку');
            })
            .finally(() => {
